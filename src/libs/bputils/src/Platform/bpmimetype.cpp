...
const std::string bp::mimetype::kFolderMimeType("application/x-folder");
...
// yajl parsing of config file to avoid using bp::Object
...
{
public:
    YajlContext() : m_state(eIgnoring), m_arrayDepth(0) {}
    tYajlState m_state;
    size_t m_arrayDepth;
    string m_ext;
    string m_type;
...
null_cb(void* ctx)
{
    YajlContext* yc = (YajlContext*)ctx;
    return yc->m_state == eIgnoring;
}
...
{
    YajlContext* yc = (YajlContext*)ctx;
    if (yc->m_state == eExpectExt) {
        yc->m_ext.assign((const char*)stringVal, (size_t)stringLen);
        yc->m_state = eExpectType;
    } else if (yc->m_state == eExpectType) {
        yc->m_type.assign((const char*)stringVal, (size_t)stringLen);
        yc->m_state = eExpectListEnd;
        set<string>& s = yc->m_map[yc->m_ext];
        s.insert(yc->m_type);
    }
    return 1;
}
...
{
    YajlContext* yc = (YajlContext*)ctx;
    if (!strncasecmp((const char*)key, "LocalMimeTypes", keyLen)) {
        yc->m_state = eExpectListofLists;
    }
    return 1;
}
...
start_array_cb(void* ctx)
{
    ...
    case eIgnoring:
        break;
    case eExpectListofLists:
        if (yc->m_arrayDepth != 1) {
            return 0;
        }
        yc->m_state = eExpectList;
        break;
    case eExpectList:
        if (yc->m_arrayDepth != 2) {
            return 0;
        }
        yc->m_state = eExpectExt;
        break;
    case eExpectExt:
    case eExpectListEnd:
    case eExpectType:
...
initialize()
{
    ...
    // add types from config file
    bp::file::Path configFilePath = bp::paths::getConfigFilePath();
    string json;
    if (bp::strutil::loadFromFile(configFilePath, json)) {
        YajlContext ctx;
        yajl_parser_config cfg = { 1, 1 };
        yajl_status ys;
        yajl_handle yh = yajl_alloc(&callbacks, &cfg, NULL, (void*)&ctx);
        ys = yajl_parse(yh, (const unsigned char*)json.c_str(), json.length());
        if (ys == yajl_status_ok) {
            map<string, set<string> >::const_iterator it;
            for (it = ctx.m_map.begin(); it != ctx.m_map.end(); ++it) {
                set<string>& s = s_extensionMap[it->first];
                const set<string>& ts = it->second;
            ...
            BPLOG_WARN_STRM("Unable to parse LocalMimeTypes from: " << json);
        }
    }
    initialized = true;
}
...
set<string> 
bp::mimetype::fromPath(const bp::file::Path& path) 
{
    initialize();
    set<string> rval;

    // chase links
    bp::file::Path target(path);
    if (bp::file::linkExists(path)) {
        if (!bp::file::resolveLink(path, target)) {
            // broken link
    ...
    string ext = bp::file::utf8FromNative(target.extension());
    if (!ext.empty()) {
...
vector<string>
bp::mimetype::extensionsFromMimeType(const string& mimeType)
{
    vector<string> rval;
    initialize();
...
bool 
bp::mimetype::pathMatchesFilter(const bp::file::Path& path,
                                const set<string>& mimeTypes)
{
    if (mimeTypes.empty()) {
        return true;
    }
    set<string> pathTypes = fromPath(path);
    set<string>::const_iterator it;
    for (it = pathTypes.begin(); it != pathTypes.end(); ++it) {
        if (mimeTypes.count(*it) > 0) {
            return true;
        }
    }
    return false;
