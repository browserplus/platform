function(platformArg, browserArg, id, cbObj) {
    ...
    var extractDragItems = function(event) {
        ...
        // and here's what we can support
        if (browser == 'Titanium') {
            // use Ti apis to get path/url pair
            for (var i = 0; i < files.length; i++) {
                var f = Titanium.Filesystem.getFile(files[i].path);
                uris[i] = f.toURL();
            }
        } else if (platform == 'Windows' && browser == 'Firefox') {
            // doze ffx exposes urls in 'text/x-moz-url'
            for (var i = 0; i < dt.mozItemCount; i++) {
                uris[i] = dt.mozGetDataAt('text/x-moz-url', i);
            }
//      } else if (platform == 'OSX' && browser == 'Firefox') {
            // XXX osx ffx requires elevated privs to get anything useful.
            // XXX Results in user seeing an alert.
...
//          }
            ...
            // osx safari exposes urls via 'text/uri-list'
            var uriText = dt.getData('text/uri-list');
            if (uriText) uris = uriText.split('\n');
        } else {
            throw 'unsupported browser/platform combo' + browser + '/' + platform;
        }
