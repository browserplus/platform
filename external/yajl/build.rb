#!/usr/bin/env ruby

require '../common'

# Get command line args
parseArgs()

topDir = File.dirname(File.expand_path(__FILE__))
buildDir = File.join(topDir, "build")

version = "1.0.4"
pkg = "yajl-#{version}"
pkgDir = File.join(topDir, pkg)
tarball = "#{pkg}.tar.gz"
md5 = "5c955747bab9f70fd922aef9d54a5590"
url = "http://github.com/lloyd/yajl/tarball/#{version}"

# Make a stab at determining if we need to build.
if !buildNeeded?("#{$headerInstallDir}/yajl/yajl_common.h")
    puts "yajl doesn't need to be built"
    exit 0
end

# Start with a clean slate
#
puts "removing previous build artifacts..."
FileUtils.rm_f("pax_global_header")
FileUtils.rm_rf(pkgDir)
FileUtils.rm_f("#{pkg}.tar")
FileUtils.rm_rf(buildDir)
if $clean
    FileUtils.rm_f(tarball)
    exit 0
end

# Fetch distro if needed
#
fetch(tarball, url, md5)

# unpack distro
#
unpack(tarball)

# since tarball came from github, it unpacks into a whacky name
whackyName = Dir.glob("lloyd-yajl-*")[0]
if whackyName && File.exists?(whackyName)
    FileUtils.rm_rf(pkgDir)
    FileUtils.mv(whackyName, pkgDir)
end

# apply patches
#
applyPatches(pkgDir)

# build
#
FileUtils.mkdir_p(buildDir)
Dir.chdir(buildDir) do
    $buildTypes.each() do |buildType|
        cmLine = "cmake #{$cmakeGenerator} -DCMAKE_BUILD_TYPE:STRING=#{buildType} \"#{pkgDir}\""
        system(cmLine)
        if $platform == "Windows"
            system("devenv YetAnotherJSONParser.sln /Build #{buildType}")
        else
            system("make")
        end

        # copy static lib
        libName = ""
        if $platform == "Windows"
            libName = File.join(buildDir, pkg, "lib", buildType, "yajl_s.lib")
        else
            libName = File.join(buildDir, pkg, "lib", "libyajl_s.a")
        end
        FileUtils.mkdir_p("#{$libInstallDir}/#{buildType}",
                          :verbose => $verbose)
        FileUtils.cp(libName, "#{$libInstallDir}/#{buildType}",
                     :verbose => $verbose)
    end
end

# copy headers 
FileUtils.mkdir_p("#{$headerInstallDir}/yajl")
Dir.glob(File.join(buildDir, pkg, "include", "yajl", "*.h")) do |x|
    FileUtils.cp(x, "#{$headerInstallDir}/yajl", :verbose => $verbose)
end

# and command line tools
FileUtils.mkdir_p($binInstallDir)
Dir.glob(File.join(buildDir, pkg, "bin", "*")) do |x|
  FileUtils.cp(x, $binInstallDir, :verbose => $verbose)
end

puts "cleaning up..."
FileUtils.rm_rf(pkgDir) if !$keepSource
FileUtils.rm_f("#{pkg}.tar")
FileUtils.rm_rf(buildDir)
